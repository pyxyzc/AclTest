...
| `host_physical_ipc_probe` | Host NUMA physical memory 的单 handle IPC 双向校验；child import/map 成功后追加 imported host VA 与 `aclrtMalloc` device ptr 的 H2D/D2H 探测；两 handle IPC 中 parent/child 各执行一次 VA-to-VA H2H |
